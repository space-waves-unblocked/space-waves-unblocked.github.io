...
function microsoftMigration() {
  ...
  if (currentHref.indexOf(newDomainOrigin) !== -1) {
    ...
    isIndexedDBAlreadyExist().then((result) => {
      if (result) {
        console.warn(
          "Migration: Stopped on new domain. IndexedDB already exist!"
        );
      } else {
  ...
  // PROVIDER MODE: Running on the old domain
    ...
    window.addEventListener("message", (eMsg) => {
      ...
      getResponse(eMsg)
        .then((result) => {
          if (result) eMsg.source.postMessage(result, eMsg.origin);
          else console.warn("Error getResponse result is NULL!");
        })
        .catch((error) => {
  ...
  function getPlayerPrefsUnity() {
    return new Promise((resolve, reject) => {
      getPlayerPrefsUnityIndexedDB()
        .then((result) => {
          const items = result.items.map(({ key, value }) => ({
            [key]: value,
          }));
  ...
  function parsePlayerPrefsBinary(data) {
    ...
    // Read header
    result.signature = decoder.decode(
      new Uint8Array(data.buffer, offset, SIGNATURE.length)
    );
      ...
      switch (type) {
        ...
        case 0x80: // Long string value
          ...
          value = decoder.decode(
            new Uint8Array(data.buffer, offset, valueLength)
          );
          offset += valueLength;
